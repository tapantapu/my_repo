...
public class AccountSharedServiceImpl implements AccountSharedService {
	...
	private static final Logger LOG = LoggerFactory.getLogger(AccountSharedServiceImpl.class);
    ...
    public boolean isLocked(String username) {
    	int lockingThresholdDb = Integer.valueOf(configService.findByName("lockingThreshold").get().getValue());
    	int lockingDurationSecondsDb = Integer.valueOf(configService.findByName("lockingDurationSeconds").get().getValue());
    	
        List<FailedAuthentication> failureEvents = authenticationEventSharedService
                .findLatestFailureEvents(username, lockingThresholdDb);
        
       // LOG.debug("lockingThresholdProp = {} lockingThresholdDb = {}  ",lockingThreshold,lockingThresholdDb);
       // LOG.debug("lockingDurationSecondsProp = {} lockingDurationSecondsDb = {}  ",lockingDurationSeconds,lockingDurationSecondsDb);
       LOG.debug("lockingThresholdLimit = {} ",lockingThresholdDb);
       LOG.debug("lockingDurationSeconds = {}  ",lockingDurationSecondsDb);
       
        if (failureEvents.size() < lockingThresholdDb) {
            return false;
        }

        if (failureEvents
                .get(lockingThresholdDb - 1)
                .getAuthenticationTimestamp()
                .isBefore(
                        dateFactory.newTimestamp().toLocalDateTime()
                                .minusSeconds(lockingDurationSecondsDb))) {
            return false;
        }
    ...
    public LocalDateTime getLastLoginDate(String username) {
        ...
        if (events.isEmpty()) {
        	...
        	LOG.debug("LastLoginDate = {}  ",events.get(0).getAuthenticationTimestamp());
    ...
    public boolean isCurrentPasswordExpired(String username) {
    	...
    	int passwordLifeTimeSecondsDb = Integer.valueOf(configService.findByName("passwordLifeTimeSeconds").get().getValue());
    	 
    	//LOG.debug("PasswordLifeTimeSecondsProp = {} passwordLifeTimeSecondsDb ={} ",passwordLifeTimeSeconds,passwordLifeTimeSecondsDb);
    	LOG.debug("PasswordLifeTimeSeconds = {} ",passwordLifeTimeSecondsDb);
        ...
        if (passwordHistories
                ...
                .isBefore(
                        dateFactory.newTimestamp().toLocalDateTime()
                                .minusSeconds(passwordLifeTimeSecondsDb))) {
            return true;
        }
    ...
    public boolean updatePassword(String username, String rawPassword) {
    	LOG.debug("Username = {} UpdatePassword ={} ",username,rawPassword);
    ...
    public String create(Account account, String imageId) {
        ...
        LOG.debug("Create Account UserName = {} Password ={} Body ={} Extension ={}",account.getUsername(),rawPassword,image,StringUtils.getFilenameExtension(tempFile
                .getOriginalName()));
        return rawPassword;
    }
